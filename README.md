...
<p align="center">
  <a href="https://github.com/zeriong/plan-smith/releases"><img src="https://img.shields.io/badge/version-1.4.2-blue" alt="Version"></a>
...
**Does the token cost pay for itself?** Measured, not estimated — cost to a *working* artifact, repair loop included (weak implementer, fixed repair-prompt template, n=4 chains per arm): the plan-smith plan cost **3.0× more to write** (689k vs 229k — it reads four skill documents), then earned it back twice over: implementations came out **42% cheaper** (a 5-file flat structure with copyable glue means the implementer reads and writes less), and repairs came out **13.5× cheaper** (620k vs 8,380k), because the defect classes differ — plan-smith's one failure was a single symbol its copyable blocks had missed (a one-line fix, and that chain ended in the test family's first stage CLEAR), while the baseline's failures were cross-component contract mismatches (one interface rebuild, one three-round whack-a-mole). Total to four working artifacts: **5.38M vs 15.58M tokens — 65.5% cheaper**. In round numbers: baseline 100 + repairs 116 = 216; plan-smith 66, done. Honest limits, recorded in the lab: tiny repair sample, one task, one weak implementer; with a strong implementer the gap shrinks (strong models succeed without the skill). Full data: z-lab `plan-smith-lab/tco/`.
